...
async function getEvent(id, db = mariadb) {
    const conn = await getConnection(db);
    if (conn) {
        const sql = 'SELECT e.id, e.title, e.description, e.location, e.likes, e.datetime_added, c.comments FROM events e INNER JOIN comments c ON e.id = c.event_id WHERE id = ?;';
